...
import com.thoughtworks.deeplearning.Compute.Compute
...
object DifferentiableKernel {
  ...
  trait DifferentiableExpression {
    ...
    def compile(context: OpenCL.Context, device: Device, commandQueue: CommandQueue)(
        implicit executor: ExecutorService): Compute[(Int, Map[Any, Tape]) => Compute[
      Tape.Aux[OpenCL.Buffer[OutputElementData], OpenCL.Buffer[OutputElementDelta]]]] = {

      def forwardProgram =
        throwableMonadic[Compute] {
          Compute(())(executor).each
          ...
          new Task(program.build().map(\/-(_))).each
          (program, inputSetter)
        }
...
//          : Compute[Tape.Aux[OpenCL.Buffer[OutputElementData], OpenCL.Buffer[OutputElementDelta]]] = {}
//      }
    }
